...
const downloadSubtitlesForMovie = async (movie, langCode) => {
  ...
  if (srtContent) {
    ...
    try {
      ...
      if (langCode === 'en' && isAutoTranslate) {
        let targetLangs = [];
      ...
      if (langCode === 'en' && isAutoTranslate) {
        ...
        for (const lang of targetLangs) {
          ...
          try {
            const translated = await translateWithProvider(cleanContent, lang);
            fs.writeFileSync(targetSubPath, translated);
            eventBus.success(`Subtitle translated: ${label} (${lang})`, { title: label, language: lang });
          } catch (translateErr) {
...
const autoTranslateExisting = async () => {
  ...
  const translateOrNativeMovie = async (fileBase, movie, lang, enSrtContent) => {
    ...
    try {
      ...
      fs.writeFileSync(targetSubPath, translated);
      eventBus.success('Subtitle translated', { title: movie.title, language: lang });
    } catch (e) { console.error(`[SubtitleService] Auto-translate failed for ${movie.title}:`, e.message); }
  };
  ...
  const translateOrNativeEpisode = async (fileBase, show, ep, lang, enSrtContent) => {
    ...
    try {
      ...
      fs.writeFileSync(targetSubPath, translated);
      eventBus.success('Subtitle translated', { title: label, language: lang });
    } catch (e) { console.error(`[SubtitleService] Auto-translate failed for ${label}:`, e.message); }
  };
  ...
  const episodes = db.prepare(`
    ...
    WHERE e.status = 'downloaded' AND e.file_path IS NOT NULL
  `).all();
  await runWithConcurrency(episodes, 3, async (ep) => {
    try {
      if (!fs.existsSync(ep.file_path)) return;
      const parsedPath = path.parse(ep.file_path);
      const enSubPath = path.join(parsedPath.dir, `${parsedPath.name}.en.srt`);
