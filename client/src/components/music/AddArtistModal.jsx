...
import { Search, Plus, Music2, Check, Loader2, Globe, Folder, ShieldCheck, Disc, Mic2 } from 'lucide-react';
import toast from 'react-hot-toast';
import api from '../../lib/api';
import ModalShell from '../shared/ModalShell';
import MusicPathsModal from './MusicPathsModal';
...
export default function AddArtistModal({ open, onClose, onAdded }) {
  ...
  const initData = async () => {
    try {
      const [profileRes, pathsRes] = await Promise.allSettled([
      ...
      if (profileRes.status === 'fulfilled' && profileRes.value.data.status === 'success') {
        const profs = profileRes.value.data.data || [];
        setProfiles(profs);
        if (profs.length > 0 && !selectedProfile) setSelectedProfile(String(profs[0].id));
      }
      ...
      if (pathsRes.status === 'fulfilled' && pathsRes.value.data.status === 'success') {
        ...
        const pathsToUse = musicPaths.length > 0 ? musicPaths : allPaths;
        setRootPaths(pathsToUse);
        if (pathsToUse.length > 0) {
          setSelectedPath((prev) => (pathsToUse.some(p => p.path === prev) ? prev : pathsToUse[0].path));
        }
      }
    } catch (err) {
    ...
    try {
      if (searchType === 'artist') {
        const payload = {
          ...
          qualityProfileId: selectedProfile ? parseInt(selectedProfile, 10) : null,
          rootFolderPath: selectedPath || null,
        };
  ...
  return (
    ...
    >
      <div className="space-y-4">
        ...
        {/* Selected item configuration options */}
        {selectedItem && (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-3 py-2 bg-slate-900/80 border border-slate-700/60 rounded-xl text-xs text-slate-200 focus:outline-none focus:border-cyan-500/80"
                >
                  {profiles.map((p) => (
                    <option key={p.id} value={p.id}>
                      {p.name} ({p.preferred_format})
                    </option>
                  ))}
                  {profiles.length === 0 && (
                    <option value="">Default (FLAC)</option>
                  )}
                </select>
              </div>
              ...
              <div>
                ...
                {rootPaths.length === 0 ? (
                  <button
                    type="button"
                    onClick={() => setPathsModalOpen(true)}
                    className="w-full px-3 py-2 bg-slate-900/80 border border-dashed border-cyan-500/40 hover:border-cyan-400 rounded-xl text-xs text-cyan-300 text-left flex items-center justify-between transition-colors"
                  >
                    <span className="truncate">Click to add mount (/mnt/oblivion/muziek)</span>
                  ...
                  <select
                    ...
                    className="w-full px-3 py-2 bg-slate-900/80 border border-slate-700/60 rounded-xl text-xs text-slate-200 focus:outline-none focus:border-cyan-500/80 truncate"
                  >
                    {rootPaths.map((p) => (
